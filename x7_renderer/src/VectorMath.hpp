...
namespace RCTGen {
    struct Vector2 {
        ...
        constexpr Vector2 operator+(Vector2 rhs) const noexcept {
            return {x + rhs.x, y + rhs.y};
        }
        constexpr Vector2 operator-(Vector2 rhs) const noexcept {
            return {x - rhs.x, y - rhs.y};
        }
        constexpr Vector2 operator*(float s) const noexcept {
            return {x * s, y * s};
        }
        constexpr float dot(Vector2 rhs) const noexcept {
            return x * rhs.x + y * rhs.y;
        }
    ...
    struct Vector3 {
        ...
        static constexpr Vector3 splat(float v) noexcept {
            return {v, v, v};
        }

        constexpr Vector3 operator+(Vector3 rhs) const noexcept {
        ...
        constexpr float dot(Vector3 rhs) const noexcept {
            return x * rhs.x + y * rhs.y + z * rhs.z;
        }
        ...
        [[nodiscard]] Vector3 normalized() const noexcept {
            const float n = norm();
            if (n == 0.0f) return {0.0f, 0.0f, 0.0f};
            return *this * (1.0f / n);
        }
    };
    ...
    constexpr Vector3 vector3_from_scalar(float a) noexcept {
        return Vector3::splat(a);
    }

    constexpr Vector2 vector2_add(Vector2 a, Vector2 b) noexcept {
        return a + b;
    }
    constexpr Vector2 vector2_sub(Vector2 a, Vector2 b) noexcept {
        return a - b;
    }
    constexpr Vector2 vector2_mult(Vector2 a, float b) noexcept {
        return a * b;
    }
    constexpr float vector2_dot(Vector2 a, Vector2 b) noexcept {
        return a.dot(b);
    }
    inline float vector2_norm(Vector2 a) noexcept {
        return a.norm();
    }

    constexpr Vector3 vector3_add(Vector3 a, Vector3 b) noexcept {
        return a + b;
    }
    constexpr Vector3 vector3_sub(Vector3 a, Vector3 b) noexcept {
        return a - b;
    }
    constexpr Vector3 vector3_mult(Vector3 a, float b) noexcept {
        return a * b;
    }
    constexpr float vector3_dot(Vector3 a, Vector3 b) noexcept {
        return a.dot(b);
    }
    constexpr Vector3 vector3_cross(Vector3 a, Vector3 b) noexcept {
        return a.cross(b);
    }
    inline float vector3_norm(Vector3 a) noexcept {
        return a.norm();
    }
    inline Vector3 vector3_normalize(Vector3 a) noexcept {
        return a.normalized();
    }
    ...
    constexpr Matrix3 matrix_identity() noexcept {
        return matrix(1, 0, 0, 0, 1, 0, 0, 0, 1);
    }
    constexpr float matrix_determinant(Matrix3 m) noexcept {
    ...
    constexpr Transform transform(Matrix3 m, Vector3 v) noexcept {
        return {m, v};
    }
